...
>
    ...
    >
        <span class="min-w-0">
            <span class="block text-base font-bold text-gray-900 dark:text-white sm:text-lg">{{ $title }}</span>
            @if($description)
                <span class="mt-1 block text-xs leading-5 text-gray-500 dark:text-gray-400 sm:text-sm">{{ $description }}</span>
            @endif
        </span>
        <span class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition dark:bg-slate-800 dark:text-slate-300" :class="sectionOpen ? 'rotate-180' : ''" aria-hidden="true">
    ...
    <div
        ...
        class="border-t border-gray-200 p-4 dark:border-gray-700 sm:p-6"
    >
        {{ $slot }}
    </div>
</section>
