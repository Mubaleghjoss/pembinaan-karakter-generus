    ...
    <form action="{{ route('materi.update', $materi) }}" method="POST" enctype="multipart/form-data" class="pkg-panel">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="materi_folder_id" id="materi_folder_id" class="w-full px-3 py-2 pkg-field">
                    ...
                    @foreach($materiFolders as $folder)
                        <option value="{{ $folder->id }}" @selected((int) old('materi_folder_id', $materi->materi_folder_id) === $folder->id)>{{ $folder->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $materi->video_url) }}" class="w-full px-3 py-2 pkg-field" placeholder="https://www.youtube.com/watch?v=...">
            </div>
