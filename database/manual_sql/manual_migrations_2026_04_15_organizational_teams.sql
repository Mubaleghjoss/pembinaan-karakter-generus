...
-- Covers:
-- - 2026_04_15_090000_create_organizational_teams_table
-- - 2026_04_15_091000_add_organizational_fields_to_users_table
--
-- Run this once on the production database before opening /pamong.
-- It is written to be safe to re-run on MySQL/MariaDB.
...
SET @sql := (
    ...
    WHERE TABLE_SCHEMA = @db_name
      ...
      AND COLUMN_NAME = 'organizational_team_id'
);
...
SET @sql := (
    ...
    WHERE TABLE_SCHEMA = @db_name
      AND TABLE_NAME = 'users'
      AND COLUMN_NAME = 'organizational_title'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql := (
    ...
    WHERE TABLE_SCHEMA = @db_name
      AND TABLE_NAME = 'users'
      AND COLUMN_NAME = 'organizational_sort_order'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql := (
    ...
    WHERE TABLE_SCHEMA = @db_name
      AND TABLE_NAME = 'users'
      AND INDEX_NAME = 'users_organizational_team_id_index'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql := (
    ...
    WHERE TABLE_SCHEMA = @db_name
      AND TABLE_NAME = 'users'
      AND CONSTRAINT_NAME = 'users_organizational_team_id_foreign'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
WHERE NOT EXISTS (
    SELECT 1 FROM `migrations` WHERE `migration` = '2026_04_15_090000_create_organizational_teams_table'
);

INSERT INTO `migrations` (`migration`, `batch`)
SELECT '2026_04_15_091000_add_organizational_fields_to_users_table', @next_batch
WHERE NOT EXISTS (
    SELECT 1 FROM `migrations` WHERE `migration` = '2026_04_15_091000_add_organizational_fields_to_users_table'
);
